...
def getIntersection(keshi, buwei, question):
    ...
    if len(Inter_list) > MAX_INTER_LENGTH:
        ...
        for word in Inter_list:
            ...
            提取关键词后进行相似度计算
            """
            similariy = smiliarity.bert_cos_similarity(key_word, word)
            print(key_word, word, similariy)
...
def getNextPath(path, text):
    ...
    if "\"" in data[2]:
        endEntity = str(data[2]).strip("\"")
        leval1 = path[endEntity]
    else:
        leval1 = path[data[2]]
...
def find_relation(question, entity):
    ...
    if '是' in relstion_result:   # 此时子图信息足够支撑用户回答，下面将该时刻的子图作为提示交给大模型进行回答
        ...
        answer = (FinallAnswer(question, leval1KP))
        end_time = time.time()
        ...
        prim_prompt = f"""请按照用户问题的顺序，从给定的列表中推理出下一级知识路径，选择符合逻辑的一个元素直接返回，请注意，返回的内容必须是列表中的元素，不能随意改变列表中元素，并且符合逐级推理的要求。
                            用户问题：{question}
                            Let’s think step by step，选择符合逻辑的下一级路径信息，并从下面的列表中选择并返回：{leval1KP}
                            另外，请不要返回list列表格式，直接返回列表中带有箭头的知识路径完整子元素，无需进行额外的分析与回复。
                            """
    ...
    # 请直接回复现在已有的关系是否支撑用户问答，完全可以则回复”是“，如果有不能回答的则必须回复”否“。
        ...
        if '是' in relstion_2result:  # 此时子图信息足够支撑用户回答，下面将该时刻的子图作为提示交给大模型进行回答
            ...
            pre2pathKnowledge = leval1KP+leval2KP
            # start_time = time.time()
            ...
            prim_prompt = f"""
                ...
                另外，请直接返回字符串答案，不要返回list列表格式的数据，无需进行额外的分析与回复。
